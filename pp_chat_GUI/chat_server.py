...
# ==============================================================================
                ...
                m = "[" + from_name + "] " + msg["message"]
                ctime = time.strftime('%d.%m.%y,%H:%M', time.localtime())
                self.indices[from_name].add_msg("(" + ctime + ") " + from_name + " : " + msg["message"] + "\n")
                line_at = self.indices[from_name].total_msgs - 1
                self.indices[from_name].indexing(msg["message"], line_at)
                ...
                for g in the_guys:
                    ...
                    ctime = time.strftime('%d.%m.%y,%H:%M', time.localtime())
                    self.indices[g].add_msg("(" + ctime + ") " + from_name + " : " + msg["message"] + "\n")
                    line_at = self.indices[g].total_msgs - 1
                    self.indices[g].indexing(msg["message"], line_at)
                    mysend(to_sock,json.dumps({"action":"exchange","from":from_name,"result":m}))
...
# ==============================================================================
                ...
                #the_guys = self.group.list_me(from_name)[:]
                #for g in the_guys:
                s = self.indices[from_name].search(msg["target"])
                if s != []:
                    for i in s:
                        search_rslt += i[1]
